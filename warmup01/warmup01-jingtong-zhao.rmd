...
description = c('gender','species','eyecolor','skincolor')
value = c('male','human','haze','light')
Han = c(description,value)
Han_matrix = matrix(Han,nrow=4,byrow=FALSE)
colnames(Han_matrix) = c('Description','Value')
knitr::kable(Han_matrix, type = 'html',caption = "Han Solo")
...
There's **no** special season of the year for Crispy Rice and Egg!!You can cook it every day!!
You can replace carrots and cucumbers with any vegetables you like, such as potatoes, mushrooms, and tomatoes.
...
\[
\begin{aligned}
d(\mathbf{p,q})=d(\mathbf{q,p}) = &\sqrt{(q_1-p_1)^2 +(q_2-p_2)^2+ \dots+ (q_n-p_n)^2}\\= &\sqrt{\sum_{i=1}^{10} (q_i-p_i)^2}
\end{aligned}
\]
The position of a point in a Euclidean n-space is a Euclidean vector. So, **p** and **q** may be represented as Euclidean vectors, starting from the origin of the space (initial point) with their tips (terminal points) ending at the two points. The Euclidean norm, or **Euclidean length**, or **magnitude** of a vector measures the length of the vector:
\[
\Vert{\mathbf{p}}\Vert=\sqrt{(p_1)^2 +(p_2)^2+ \dots+ (p_n)^2} = \sqrt{\mathbf{p}\cdot \mathbf{p}}
\]
